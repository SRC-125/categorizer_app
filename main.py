...
#     # Create a dictionary to hold confidence data
#     confidence_data = {
#         "File Name": [],
#         "Category Confidence": [],
#         "Domain Confidence": []
...
#     # Loop through the metadata cache to populate confidence data
#     for file_name, meta in metadata_cache.items():
#         category_confidence = meta.get("category_confidence","Not Available")
#         domain_confidence = meta.get("domain_confidence", "Not Available")
        
#         confidence_data["File Name"].append(file_name)
#         confidence_data["Category Confidence"].append(category_confidence)
#         confidence_data["Domain Confidence"].append(domain_confidence)

#     # Convert confidence data to a DataFrame
#     confidence_df = pd.DataFrame(confidence_data)

#     # Display the confidence scores in the tab
...
# from agents.reader_agent import run_reader_agent
# from agents.categorizer_agent import run_categorization_agent
# from agents.validation_agent import validate_domain_confidence, validate_category_confidence  # Import validation agent
...
#     # Assuming confidence scores are returned by the categorization agent
#     category_confidence_score = agent_output.get("category_confidence", 1.0)  # Default to 1.0 if not found
#     domain_confidence_score = agent_output.get("domain_confidence", 1.0)  # Default to 1.0 if not found
    
#     # Validate category and domain confidence scores
#     is_category_valid = validate_category_confidence(category, category_confidence_score)
#     is_domain_valid = validate_domain_confidence(domain, domain_confidence_score)

#     # If both category and domain are valid, proceed with saving metadata
#     if is_category_valid and is_domain_valid:
#         result = {
...
#         return result
#     else:
#         st.warning(f"Validation failed for {file_name}. Category or domain extraction is not confident.")
